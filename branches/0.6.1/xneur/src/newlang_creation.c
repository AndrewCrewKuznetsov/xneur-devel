...
static void lower_by_keymap(char *text, int group)
{
	...
	for (i = 0; i < text_len; i++)
	{
		...
		for (j = main_window->xkeymap->min_keycode + 1; j <= main_window->xkeymap->max_keycode; j++)
		{
			...
			if (memcmp(text + i, main_window->xkeymap->alphabet[group][j].upper_sym, strlen(main_window->xkeymap->alphabet[group][j].upper_sym)) == 0)
			{
				memcpy(text + i, main_window->xkeymap->alphabet[group][j].lower_sym, strlen(main_window->xkeymap->alphabet[group][j].lower_sym));
