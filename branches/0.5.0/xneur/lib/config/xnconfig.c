...
static enum _error parse_line(struct _xneur_config *p, char *line)
{
	...
	switch (index)
	{
		...
		{
			p->dummy_apps->add(p->dummy_apps, param);
			break;
		}
		...
		case 15: // Get config version
		{
			p->conf_version = (char *) xnmalloc((strlen(param) + 1) * sizeof(char));
			strcpy(p->conf_version, param);
			if (strcmp(param, VERSION) != 0)
				error_code = ERROR_INVALID_VERSION;
			break;
		}
		case 16: // Get Layout Remember Mode
		{
			if (strcmp(param, "Yes") == 0)
		...
		case 17: // Get Save Selection Mode
		{
			if (strcmp(param, "Yes") == 0)
...
enum _error xneur_config_load(struct _xneur_config *p)
{
	...
	for (lang = 0; lang < p->total_languages; lang++)
	{
		...
		p->languages[lang].regexp = load_file_to_list(lang_dir, REGEXP_NAME);
...
enum _error xneur_config_save(struct _xneur_config *p)
{
	FILE *stream = fopen (get_file_path_name(NULL, CONFIG_NAME), "w");
	if (stream != NULL) 
	{
		log_message(LOG, "Saving main config to %s", get_file_path_name(NULL, CONFIG_NAME));
		...
		for (int i = 0; i < p->dummy_apps->data_count; i++)
			fprintf(stream, "DummyApp %s\n", p->dummy_apps->data[i].string);
		fputs("\n", stream);
...
enum _error xneur_save_dicts (struct _xneur_config *p)
{
	...
	for (lang = 0; lang < p->total_languages; lang++)
	{
		snprintf(dict_path, 1024, "%s/%s", p->get_lang_dir_full(p, lang), DICT_NAME);
		log_message(LOG, "Saving %s dictionary to %s", p->get_lang_name(p, lang), dict_path);

		write_list_to_file(dict_path, p->languages[lang].dicts);
	}
