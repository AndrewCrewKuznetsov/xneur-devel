...
void list_rem(struct _list *list, const char *string, int string_size)
{
	...
	for (i = 0; i < list->data_count; i++)
	{
		...
		if (list->data_count != i)
			list->data[i] = list->data[list->data_count];
...
struct _list* list_init()
{
	...
	// function mapping
	element->uninit = list_uninit;
	element->add = list_add;
	element->rem = list_rem;
	element->find = list_find;
	element->exist = list_exist;
	element->exist_regexp = list_exist_regexp;
