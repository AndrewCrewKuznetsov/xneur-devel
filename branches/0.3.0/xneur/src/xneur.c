...
static void xneur_set_lock(void)
{
	...
	{
		int locked_pid = xneur_config_get_pid();
		if (locked_pid != -1)
		{
			log_message(ERROR, PROGRAM_NAME " already running with pid %d", locked_pid);
			exit(EXIT_FAILURE);
		}
	}

	int xneur_pid = getpid();
	if (xneur_config_set_pid(xneur_pid) != TRUE)
	{
		log_message(ERROR, "Can't create shared memory segment");
		exit(EXIT_FAILURE);
	}

	log_message(DEBUG, PROGRAM_NAME " pid is %d", xneur_pid);
}
...
int main(int argc, char *argv[])
{
	...
	for (lang = 0; lang < xconfig->total_languages; lang++)
	{
		...
		log_message(DEBUG, "%s proto has %d records", lang_name, xconfig->protos[lang]->data_count);
	}
