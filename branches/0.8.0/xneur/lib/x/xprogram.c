...
static int get_auto_action(struct _xprogram *p, KeySym key, int modifier_mask)
{
	...
	if (IsKeypadKey(key))
	{
		...
		{
			if ((modifier_mask & ControlMask) || (modifier_mask & Mod1Mask) 
				|| (modifier_mask & ShiftMask) || (modifier_mask & Mod4Mask))
				return KLB_CLEAR;
			return KLB_ADD_SYM;
		}
...
void xprogram_layout_update(struct _xprogram *p)
{
	...
	for (lang = 0; lang < xconfig->total_languages; lang++)
	{
		...
		int len = strlen(window_layouts);

		struct _list_char_data* data = xconfig->window_layouts->find(xconfig->window_layouts, window_layouts, len, BY_PLAIN);
		if (data != NULL)
			xconfig->window_layouts->rem(xconfig->window_layouts, window_layouts, len);
	}
	...
	for (lang = 0; lang < xconfig->total_languages; lang++)
	{
		...
		struct _list_char_data* data = xconfig->window_layouts->find(xconfig->window_layouts, window_layouts, strlen(window_layouts), BY_PLAIN);
		if (data != NULL)
		{
			switch_lang(lang);
			log_message(DEBUG, "Restore layout group to %d", xconfig->xkb_groups[lang]);
			return;
		}
	}
	...
	while (1)
	{
		...
		switch (type)
		{
			...
			case PropertyNotify:
			{
				...
				//char *aname = XGetAtomName (main_window->display, e->atom);
				//log_message(DEBUG, "Property Notify with Atom 0x%lx (%s)", e->atom, aname);
				break;
			}
			default:
			{
...
void xprogram_perform_auto_action(struct _xprogram *p, int action)
{
	...
	switch (action)
	{
		...
		case KLB_ADD_SYM:
		{
			char sym = ' ';
			if (action != KLB_ENTER)
				sym = main_window->xkeymap->get_cur_ascii_char(main_window->xkeymap, p->event->event);
...
//	XGrabKeyboard(main_window->display, p->focus->owner_window, FALSE, 
...
//	XUngrabKeyboard(main_window->display, CurrentTime);
