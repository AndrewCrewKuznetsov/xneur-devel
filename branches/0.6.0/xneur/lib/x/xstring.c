...
{
	int i;
	// Get new language bit
	int keycode_mod = get_keycode_mod(xconfig->xkb_groups[lang]);
	// Create all languages common mask
	int flush_mod;
	for (int group = 0; group < groups_size; group++)
		flush_mod = flush_mod | groups[group];
	// Set new language bit
	for (i = 0; i < p->cur_pos; i++)
	{
		// Flush old language bits
		p->keycode_modifiers[i] = p->keycode_modifiers[i] & (~flush_mod);
		//int shift_mod = p->keycode_modifiers[i] & ShiftMask;
