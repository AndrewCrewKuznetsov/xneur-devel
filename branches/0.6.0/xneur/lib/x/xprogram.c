...
void xprogram_on_key_press(struct _xprogram *p)
{
	...
	//p->event->get_key_info(p->event); // View all information of keyboard event
...
int xprogram_perform_manual_action(struct _xprogram *p, enum _hotkey_action action)
{
	switch (action)
	{
		...
		{
			log_message(DEBUG, "Requested action %d", action);
...
struct _xprogram* xprogram_init(void)
{
	...
	main_window->xkeymap = xkeymap_init();
	if (!main_window->xkeymap)
		return NULL;
