...
glib-gettextize --version > /dev/null 2>&1
if [ $? -ne 0 ]; then
        echo
        echo "**Error**: You need 'glib' version 2.12 or newer installed."
        echo "Download the appropriate package for your distribution, or get the
 source tarball at ftp://ftp.gtk.org/pub/gtk"
        exit 1
fi
...
if [ $? -ne 0 ]; then
        echo
        echo "Error while running glib-gettextize"
        exit 1
fi
