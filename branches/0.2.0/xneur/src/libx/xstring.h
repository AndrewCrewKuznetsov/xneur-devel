...
typedef struct _xstr
{
	...
	void (*clear) (struct _xstr *p);
	void (*set_key_code) (struct _xstr *p, int group);
	void (*set_content) (struct _xstr *p, const char *src);
	void (*add_symbol) (struct _xstr *p, char sym);
	void (*del_symbol) (struct _xstr *p);
	void (*uninit) (struct _xstr *p);
} xstr;
