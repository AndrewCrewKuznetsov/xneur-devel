...
GtkWidget* create_menu_icon(struct _tray_icon *tray, gboolean runned, int state)
{
	...
	if (runned == TRUE)
	{
		menu_text = _("Остановить демон");
		menu_icon = "gtk-stop";
	}
	else
	{
		menu_text = _("Запустить демон");
	...
	if (state == 0)
	{
		menu_text = _("Откл. автокоррекцию ввода");
		menu_icon = "gtk-disconnect";
	}
	else
	{
		menu_text = _("Вкл. автокоррекцию ввода");
...
void create_tray_icon(struct _tray_icon *tray, gboolean runned)
{
	...
	if (runned == TRUE)
	{
		hint = g_strdup_printf("%s%s%s", _("X Neural Switcher RUNNED ("), layout_name, ")");
