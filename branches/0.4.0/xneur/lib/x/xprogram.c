...
void save_dicts(void)
{
	...
	{
		char *dict_path = xnmalloc(1024);
		snprintf(dict_path, 1024, "%s/%s", xconfig->get_lang_dir_full(xconfig, lang), DICT_NAME);
		log_message(LOG, "Saving %s dictionary to %s", xconfig->get_lang_name(xconfig, lang), dict_path);
		
		FILE *stream;
		if ((stream = fopen (dict_path, "w")) != NULL) 
		{
			for (int i=0; i < xconfig->languages[lang].dicts->data_count; i++)
			{
				fputs(xconfig->languages[lang].dicts->data[i].string, stream);
				fputs("\n", stream);
			}
			fclose (stream);		
		}
...
int xprogram_perform_manual_action(struct _xprogram *p, enum _hotkey_action action)
{
	switch (action)
	{
		...
		{
			p->send_string_silent(p, get_next_lang(get_cur_lang()), TRUE);
			break;
		}
...
{
	// Check force mode
	int is_auto = 0, is_manual = 0;
	char *new_app_name = get_wm_class_name(p->focus->owner_window);
	if (new_app_name)
	{
		is_auto = list_exist(xconfig->auto_apps, new_app_name, strlen(new_app_name));
		is_manual = list_exist(xconfig->manual_apps, new_app_name, strlen(new_app_name));
	...
	if ((xconfig->get_current_mode(xconfig) == MANUAL_MODE) && (!is_auto))
		return;
...
void xprogram_send_string_silent(struct _xprogram *p, int new_lang, int send_backspaces)
{
...
void xprogram_change_word(struct _xprogram *p, int new_lang)
{
	...
	p->send_string_silent(p, new_lang, TRUE);
...
void xprogram_add_word_to_dict(struct _xprogram *p, int new_lang)
{
	...
	if (list_exist(xconfig->languages[curr_lang].temp_dicts, low_word, len)) 
		list_rem(xconfig->languages[curr_lang].temp_dicts, low_word, len);		
	if (!list_exist(xconfig->languages[new_lang].temp_dicts, low_word, len)) 
		list_add(xconfig->languages[new_lang].temp_dicts, low_word);
	else
	{
		if (list_exist(xconfig->languages[curr_lang].dicts, low_word, len)) 
		{
			log_message(DEBUG, "Remove word '%s' from %s dictionary", low_word, xconfig->get_lang_name(xconfig, curr_lang));
			list_rem(xconfig->languages[curr_lang].dicts, low_word, len);		
		}
		if (!list_exist(xconfig->languages[new_lang].dicts, low_word, len)) 
		{
			log_message(DEBUG, "Add word '%s' in %s dictionary", low_word, xconfig->get_lang_name(xconfig, new_lang));
			list_add(xconfig->languages[new_lang].dicts, low_word);
		}
	}	
}
